...
namespace BLL
{
    public class ExcelExport
    {
        ...
        public const string T_DSNhanVien = "HienThi";
        public const string T_DSNCC = "HienThiDSNCC";
        public const string T_DSLoaiHang = "HienThiDSL";
        public const string T_DSHang = "HienThiDSH";
        public const string T_DSTaiKhoan = "HienThiDSTK";
        public const string T_DSKhachHang = "HienThiDSKH";
        public const string T_HOADON = "HienThiHD";
        public const string T_PHIEUNHAP = "HienThiPN";
        #endregion
        ...
        private void BuildReplacerCurrentDateHOADON(ref Dictionary<string, string> pReplacer, string mahd, string ngaylap, string kh, string nv)
        {
            if (pReplacer != null)
            {
                ...
                pReplacer.Add("%Khach", kh);
                //pReplacer.Add("%ThanhTien", tongtien.ToString());
            }
        }
        private void BuildReplacerCurrentDatePhieuNhap(ref Dictionary<string, string> pReplacer, string mapn, string ngaylap, string nv)
        {
            if (pReplacer != null)
            {
                pReplacer.Add("%MaPN", mapn);
                pReplacer.Add("%NgayThangNam", ngaylap);
                pReplacer.Add("%NhanVien", nv);
                //pReplacer.Add("%ThanhTien", tongtien.ToString());
            }
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
            ...
            switch (viewName)
            {
                ...
                case T_DSNhanVien:
                    ...
                    break;
                case T_DSTaiKhoan:
                    arrByte = File.ReadAllBytes("DanhSachTaiKhoan.xls").ToArray();
                    break;
                case T_DSNCC:
                    arrByte = File.ReadAllBytes("DanhSachNCC.xls").ToArray();
                    break;
                case T_DSLoaiHang:
                    arrByte = File.ReadAllBytes("DanhSachLoaiHang.xls").ToArray();
                    break;
                case T_DSKhachHang:
                    arrByte = File.ReadAllBytes("DanhSachKH.xls").ToArray();
                    break;
                case T_DSHang:
                    arrByte = File.ReadAllBytes("DanhSachHang.xls").ToArray();
                    break;
                case T_HOADON:
        ...
        //}
        public bool ExportDSNhanVien(List<INDSNHANVIEN> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
        ...
        public bool ExportDSTaiKhoan(List<INDSTAIKHOAN> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            return OutSimpleReport(dataSource, replacer, "HienThiDSTK", isPrintPreview, ref fileName);
        }
        public bool ExportDSHang(List<INDSHANG> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            return OutSimpleReport(dataSource, replacer, "HienThiDSH", isPrintPreview, ref fileName);
        }
        public bool ExportDSKH(List<INDSKHACHHANG> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            return OutSimpleReport(dataSource, replacer, "HienThiDSKH", isPrintPreview, ref fileName);
        }
        public bool ExportDSNCC(List<INDSNCC> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            return OutSimpleReport(dataSource, replacer, "HienThiDSNCC", isPrintPreview, ref fileName);
        }
        public bool ExportHOADON(List<INHOADON> dataSource, ref string fileName, bool isPrintPreview,string mahd,string ngaylap,string nv,string kh)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            BuildReplacerCurrentDateHOADON(ref replacer,mahd,ngaylap,kh,nv);

            return OutSimpleReport(dataSource, replacer, "HienThiHD", isPrintPreview, ref fileName);
        }
        public bool ExportPhieuNhap(List<INPHIEUNHAP> dataSource, ref string fileName, bool isPrintPreview, string mapn, string ngaylap, string nv)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            BuildReplacerCurrentDatePhieuNhap(ref replacer, mapn, ngaylap, nv);

            return OutSimpleReport(dataSource, replacer, "HienThiPN", isPrintPreview, ref fileName);
        }
        #endregion
